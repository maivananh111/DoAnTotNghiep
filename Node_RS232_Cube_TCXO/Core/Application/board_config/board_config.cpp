...
bool board_config_parse(board_config_t *pconfig, const char *json_str, uint16_t json_len){
    ...
    if(!cJSON_HasObjectItem(root, "wan") || !cJSON_HasObjectItem(root, "mpn")) return false;
    cJSON *wan = cJSON_GetObjectItem(root, "wan");

    char *wan_str = cJSON_PrintUnformatted(wan);
    char *mpn_str = cJSON_GetObjectItem(root, "mpn")->valuestring;
    uint16_t mpn_len = strlen(mpn_str);
    pconfig->mpn = (char *)malloc(mpn_len + 1);
    memcpy(pconfig->mpn, mpn_str, mpn_len);
    pconfig->mpn[mpn_len] = 0;

    LOG_INFO(TAG, "LoRaWAN configuration: %s", wan_str);
    LOG_INFO(TAG, "Sensor manufacturer product number: %s", pconfig->mpn);
