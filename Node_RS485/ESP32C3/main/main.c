...
static void uart_send(char *data){
	...
	for(uint8_t i=0; i<a; i++)
		ESP_LOGE(TAG, "Send %d bytes to RAK.", uart_write_bytes(UART_NUM_1, (char *)(x + i*100), 100));

	if(b)
		ESP_LOGE(TAG, "Send %d bytes to RAK.", uart_write_bytes(UART_NUM_1, (char *)(x + a*100), b));
}
