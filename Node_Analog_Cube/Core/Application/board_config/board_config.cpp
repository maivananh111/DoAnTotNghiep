...
bool board_config_parse(board_config_t *pconfig, const char *json_str, uint16_t json_len){
    ...
    if(!cJSON_HasObjectItem(root, "wan") || !cJSON_HasObjectItem(root, "calib")) return false;
    cJSON *wan = cJSON_GetObjectItem(root, "wan");
    cJSON *calib = cJSON_GetObjectItem(root, "calib");

    char *calib_str = cJSON_PrintUnformatted(calib);
    char *wan_str = cJSON_PrintUnformatted(wan);
    uint16_t calib_len = strlen(calib_str);
    pconfig->calib = (char *)malloc(calib_len + 1);
    memcpy(pconfig->calib, calib_str, calib_len);
    pconfig->calib[calib_len] = 0;

    LOG_INFO(TAG, "LoRaWAN configuration: %s", wan_str);
    LOG_INFO(TAG, "Analog calibration: %s", pconfig->calib);
